...
{
    int n = (int)floor(x);
    if (n%2 == 0) return (float)floor(x/2.f);
    else return (x - (float)n) + (float)floor(x/2.f);
}
...
static inline float logistic_activate(float x){return 1./(1. + (float)exp(-x));}
static inline float loggy_activate(float x){return 2./(1. + (float)exp(-x)) - 1;}
static inline float relu_activate(float x){return x*(x>0);}
static inline float elu_activate(float x){return (x >= 0)*x + (x < 0)*((float)exp(x)-1);}
static inline float selu_activate(float x){return (x >= 0)*1.0507f*x + (x < 0)*1.0507f*1.6732f*((float)exp(x)-1);}
...
static inline float tanh_activate(float x){return ((float)exp(2*x)-1)/((float)exp(2*x)+1);}
